...
Village circles respect current vision: the center must be visible, and the circumference is clipped against fog and unexplored terrain. Previously explored villages do not show live rings through fog. Floating capture/income text already uses the engine fog checks. `tools/greywater/villages-fog-preview.lua` provides a visual regression scenario.
